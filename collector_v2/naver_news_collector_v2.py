...
def collect_naver_news(keyword_input, limit=100):
...
def main():
    ...
    df = collect_naver_news(keyword, limit)
    print("\nrows:", len(df))
    if not df.empty:
        print(df[["keyword", "title", "url", "published_at", "description"]].head(20))

    save = input("\nsave csv? (y/N): ").strip().lower()
    if save == "y":
        output_dir = Path(__file__).resolve().parent / "data" / "raw"
        output_dir.mkdir(parents=True, exist_ok=True)
        path = output_dir / f"naver_news_{keyword}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
