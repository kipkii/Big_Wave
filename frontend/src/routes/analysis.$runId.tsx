...
function AnalysisPage() {
  ...
  if (!data) return null;
  const weeklySeries = data.charts.weekly_partition_series ?? [];
  const hasReactionData = weeklySeries.some((point) => point.views > 0);
      ...
      <div className="mx-auto max-w-6xl px-6 py-8 space-y-6">
        ...
        {!hasReactionData && (
          <div className="rounded-xl border border-border bg-muted/35 px-4 py-3 text-sm text-muted-foreground">
            현재 선택된 데이터에는 조회수/댓글/좋아요 같은 반응 지표가 없습니다. 이 화면은 콘텐츠 공급량과 키워드 언급량 중심으로만 표시합니다.
          </div>
        )}
        ...
        {/* Component scores */}
        <section className={`grid grid-cols-2 ${hasReactionData ? "md:grid-cols-4" : "md:grid-cols-3"} gap-3`}>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card title={hasReactionData ? "Supply vs Reaction" : "콘텐츠 공급량"}>
            <ChartBox>
              {hasReactionData ? (
                <ComposedChart data={weeklySeries}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                  <XAxis dataKey="period" fontSize={11} stroke="var(--muted-foreground)" />
                  <YAxis yAxisId="left" fontSize={11} stroke="var(--muted-foreground)" />
                  <YAxis yAxisId="right" orientation="right" fontSize={11} stroke="var(--muted-foreground)" />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Bar yAxisId="left" dataKey="content_supply" fill="oklch(0.72 0.01 285)" name="콘텐츠 수" />
                  <Line
                    yAxisId="right"
                    type="monotone"
                    dataKey="views"
                ...
                <BarChart data={weeklySeries}>
                  ...
                  <Tooltip contentStyle={tooltipStyle} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Bar dataKey="content_supply" fill="var(--primary)" name="콘텐츠 수" />
                  <Bar dataKey="weighted_supply" fill="var(--primary-glow)" name="가중 콘텐츠" />
                </BarChart>
              )}
            </ChartBox>
          </Card>
